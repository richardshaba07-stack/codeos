...
impl ParserActor {
    ...
    fn publish(&self, results: Vec<ParsedFileResult>) {
        ...
        // da fare" per uno stallo. (Bug osservato su un repo reale: `arel`,
        // branch-stub con 0 sorgenti, andava in time-box a 420s.)
...
mod tests {
    ...
    async fn empty_project_still_publishes_files_indexed() {
        // Regressione (caso reale osservato): `arel` è un branch-stub
    ...
    async fn unknown_extension_yields_empty_files_indexed() {
        ...
        // (Prima emetteva "nessun evento" — il bug arel osservato su un repo reale.)
        let nanos = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos();
